namespace Service.Core.TelegramBot.RegisterCondition
{
    ...
    /// Место поиска
    /// </summary>
    public class SearchPlaceCondition : BaseRegisterCondition, ICondition, IUpdater
    {
        private bool isDone;
        ...
        public SearchPlaceCondition(DataManager dataManager, IListener listener, bool isOptional = true)
        {
            _listener = listener;
        ...
        public async Task Execute(Update update)
        {
            if (!isStarted)
            {
                ...
                await _dataManager.GetData<TelegramBotManager>().SendTextMessage(chatId, _message[MessageKey.ENTER_SEARCH_PLACE], replyKeyboard, parseMode: ParseMode.MarkdownV2);
                isStarted = true;
                _dataManager.GetData<ILogger>().Debug($"User:{userId}; Condition:{nameof(SearchPlaceCondition)} -  Start condition");
            }
        }
        ...
        public async Task<bool> CheckCondition(Update update)
        {
            ...
            {
                isDone = true;
                _dataManager.GetData<ILogger>().Debug($"User:{userId}; Condition:{nameof(SearchPlaceCondition)} -  Skip");
            }
            ...
            {
                await _dataManager.GetData<TelegramBotManager>().SendTextMessage(chatId, _message[MessageKey.WRONG_FORMAT_SEARCH_PLACE]);
                _dataManager.GetData<ILogger>().Debug($"User:{userId}; Condition:{nameof(SearchPlaceCondition)} -  Wrong format");
            }
            return isDone;
        }
